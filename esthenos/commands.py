...
class TestDBQuestions(Command):
  """initialized test cgt1, cgt2, grt and tele-calling questions"""

  def run(self):
        org = EsthenosOrg.objects.all()[0]
        orgz = settings.ORGS_SETTINGS
        questions = orgz["test_questions"]

        for question in questions["grt"]:
            EsthenosOrgGRTTemplateQuestion.objects.get_or_create(
              question=question["question"],
              question_regional=question["question_regional"],
              language_type=question["language"],
              organisation=org
            )
        notify("{count} {name} grt questions created.".format(count=len(questions["grt"]), name=orgz["name"]))

        for question in questions["cgt1"]:
            EsthenosOrgCGT1TemplateQuestion.objects.get_or_create(
              question=question["question"],
              question_regional=question["question_regional"],
              language_type=question["language"],
              organisation=org
            )
        notify("{count} {name} cgt1 questions created.".format(count=len(questions["cgt1"]), name=orgz["name"]))

        for question in questions["cgt2"]:
            EsthenosOrgCGT2TemplateQuestion.objects.get_or_create(
              question=question["question"],
              question_regional=question["question_regional"],
              language_type=question["language"],
              organisation=org
            )
        notify("{count} {name} cgt2 questions created.".format(count=len(questions["cgt2"]), name=orgz["name"]))

        for question in questions["telecalling"]:
            EsthenosOrgTeleCallingTemplateQuestion.objects.get_or_create(
              question=question["question"],
              question_regional=question["question_regional"],
              language_type=question["language"],
              organisation=org
            )
        notify("{count} {name} telecalling questions created.".format(count=len(questions["telecalling"]), name=orgz["name"]))
...
def add_commands( manager):
    ...
    manager.add_command( 'test-questions', TestDBQuestions())
