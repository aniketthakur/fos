...
@auth_views.route('/admin/employee/signup', methods=["POST"])
def developer_signup():
    if request.method == "POST":
        ...
        if(form.validate()):
            ...
            user = PUser.objects.get(id=user.get_id())
            ...
            if form.owner_id.data is not None and form.owner_id.data != "":
                owner = None
                try:
                    owner = PUser.objects.get(id = form.owner_id.data)
                except Exception,e:
                    print e.message
...
@auth_views.route('/admin/agent/signup', methods=["POST"])
def developer_signup():
    if request.method == "POST":
        register_form = RegistrationForm( request.form)
        form = register_form
