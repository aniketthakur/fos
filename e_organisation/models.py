...
class EsthenosOrgApplication(db.Document):
    ...
    sales_revenue_in_9_month = db.FloatField(default=0.0)
    ...
    required_loan_amount = data.get("required_loan_amount","")
    repayment_option = data.get("repayment_option","")
    emi_capacity = data.get("emi_capacity_as_per_the_customer","")
    type_of_repayment = data.get("type_of_repayment","")
    emi = data.get("emi","")
    tenure = data.get("tenure","")
    amount_allocated_for_assets = data.get("if_assets_purchaged_,_amount_allocated_for_assets","")
